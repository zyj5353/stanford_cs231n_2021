...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        _2j=torch.arange(0, embed_dim, step=2).float()
        pe[0,:,0::2]=torch.sin(pos /(10000 ** (_2j / embed_dim)))
        pe[0,:,1::2]=torch.cos(pos /(10000 ** (_2j/ embed_dim)))
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        q=q.view((N, S, H, E//H)).transpose(1,2)    #N*H*S*E/H
        k=k.view((N, T, H, E//H)).transpose(1,2)   # N*H*T*E/H
        v=v.view((N, T, H, E//H)).transpose(1,2)    #N*H*T*E/H
        k_t=k.transpose(2,3)                        #N*H*E/H*T
        weight=torch.matmul(q,k_t)/ math.sqrt(E//H)  #N*H*S*T
        if attn_mask is not None:
          weight=weight.masked_fill(attn_mask==0,-1e12)    #N*H*S*T
        
        weight=self.softmax(weight)
        # in order to match the autograder answer, add dropout layer here. not quite understand this part.
        weight=self.dropout(weight)
        score=torch.matmul(weight,v)    ##N*H*S*E/H
        #output=self.dropout(score)
        output=score.transpose(1,2).reshape(N,S,E)
        #output=self.dropout(output)

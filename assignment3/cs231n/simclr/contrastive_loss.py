...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for i in range(2*N):
          ...
          if i!=k+N:
            sum_kn+=torch.exp(sim(z_i,z_k_N)/tau)
        loss_k=-torch.log(torch.exp(sim(z_k,z_k_N)/tau)/sum_k)
        loss_kn=-torch.log(torch.exp(sim(z_k,z_k_N)/tau)/sum_kn)
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs=((out_left*out_right).sum(axis=1)/(left_norm*right_norm)).view(N,1)
...
def compute_sim_matrix(out):
    ...
    sim_matrix=(torch.matmul(out,out.T)/norm)/norm.T
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    pos_pairs_2=torch.cat([pos_pairs,pos_pairs],dim=0)
    numerator=torch.exp(pos_pairs_2/tau)
